 exports.FormatAop = ({ body: { aops, entities, currentEntity, filters } }) => {
    ...
    const dateToSeconds = date => {
      ...
      const numberOfProducts = entities[currentEntity].lovs.allProducts.length
      ...
      const Offers = Object.keys(entities[currentEntity].lovs.allOffers)
      const numberOfOffers = Offers.length
      const prod = []
    
      for (var i = 0; i < numberOfOffers; i++) {
        var temp = entities[currentEntity].lovs.allOffers[Offers[i]].products
        prod.push(new Array(temp.length).fill(0))
      }
      ...
      for (let i = 0; i < numberOfRecords * entities[currentEntity].part; i++) {
        ...
        if (!aopsFormatted[id]) {
          ...
          aopsFormatted[id] = {
            ...
            product: entities[currentEntity].lovs.allProducts[product],
